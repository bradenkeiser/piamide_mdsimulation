...
data <- data.frame(frames = 1:length(xyz[,1]), double = FALSE, in_dist = 0, dist = 0, in_ang =0, angle = 0,
...
for ( frame in 1:length(xyz[,1])) {
  ...
  data$normed_pos[frame] <- paste(ref[1],ref[2],ref[3],sep = ' ') #was list(ref)
  #calculate angle: normed_vec, amide_com, ring_com
  angle <- angle.xyz(c(ref, amide_pos, ring_pos)); angle
  
  data$angle[frame] <- angle
  data$in_ang[frame] <- ifelse(angle <= angle_cutoff, TRUE, FALSE)
  data$double[frame] <- ifelse(angle <= angle_cutoff & distance <= distance_cutoff, 
                               TRUE, FALSE)
} 
